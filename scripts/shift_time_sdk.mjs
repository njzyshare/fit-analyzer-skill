...
if (startEv) enc.writeMesg({ mesgNum: Profile.MesgNum.EVENT, ...shiftObj(strip(startEv), delta) });

// records
for (const r of (messages.recordMesgs || [])) enc.writeMesg({ mesgNum: Profile.MesgNum.RECORD, ...shiftObj(strip(r), delta) });

// laps
const laps = messages.lapMesgs || [];
for (let i = 0; i < laps.length; i++) {
  if (lapEvs[i]) enc.writeMesg({ mesgNum: Profile.MesgNum.EVENT, ...shiftObj(strip(lapEvs[i]), delta) });
  enc.writeMesg({ mesgNum: Profile.MesgNum.LAP, ...shiftObj(strip(laps[i]), delta) });
}
for (const e of otherEvs) enc.writeMesg({ mesgNum: Profile.MesgNum.EVENT, ...shiftObj(strip(e), delta) });
if (stopEv) enc.writeMesg({ mesgNum: Profile.MesgNum.EVENT, ...shiftObj(strip(stopEv), delta) });
